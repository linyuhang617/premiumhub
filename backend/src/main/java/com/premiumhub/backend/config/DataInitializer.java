        ...
        return args -> {
            ...
            if (paymentRepository.count() == 0) {
                ...
                for (Object[] row : payments) {
                    ...
                    p.setPolicyId((Long) row[0]);
                    p.setAmount(new BigDecimal((String) row[1]));
                    p.setStatus((String) row[2]);
                    p.setTraceId(UUID.randomUUID().toString());
                    p.setPaidAt(LocalDateTime.now());
                    paymentRepository.save(p);
                }
                System.out.println("✅ 測試請款初始化完成（3 筆 PENDING）");
            }
        };
    }
}
