...
public class PolicyService {
    ...
    public Map<String, Object> queryPolicies(PolicyQueryRequest req) {
        ...
        List<PolicyResponse> content = policies.stream().map(this::toResponse).toList();
    ...
    public PolicyResponse createPolicy(PolicyRequest req) {
        Policy policy = new Policy();
        policy.setPolicyNo(req.getPolicyNo());
        policy.setInsuredName(aesUtil.encrypt(req.getInsuredName()));
        policy.setIdNoEncrypted(aesUtil.encrypt(req.getIdNo()));
        policy.setPremiumAmount(req.getPremiumAmount());
        policy.setStatus(req.getStatus());
        policy.setDueDate(req.getDueDate());
        return toResponse(policyRepository.save(policy));
    }

    public PolicyResponse updatePolicy(Long id, PolicyRequest req) {
        Policy policy = policyRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Policy not found: " + id));
        policy.setPolicyNo(req.getPolicyNo());
        policy.setInsuredName(aesUtil.encrypt(req.getInsuredName()));
        ...
        policy.setStatus(req.getStatus());
        policy.setDueDate(req.getDueDate());
        policy.setVersion(req.getVersion());
        try {
            return toResponse(policyRepository.save(policy));
